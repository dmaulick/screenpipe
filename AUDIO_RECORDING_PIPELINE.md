...
graph TD
    ...
    B --> C[Voice Activity Detection<br/>WebRTC/Silero]
    C -->|Speech Detected| D[Audio Preprocessing<br/>16kHz Resampling]
    C -->|Silence| B
    D --> E[Speaker Segmentation<br/>PyAnnote Models]
    E --> F[Speaker Embedding<br/>WeSpeaker Model]
    F --> G[Speaker Identification<br/>Clustering]
    G --> H[Speech-to-Text<br/>Whisper/Deepgram]
    H --> I[Overlap Cleanup<br/>Deduplication]
    I --> J[Database Storage<br/>SQLite + FTS5]
    J --> K[Real-time Broadcasting<br/>WebSocket + Events]
    K --> L[Intelligence Extraction<br/>Semantic Analysis]
```
...
|-------|-------------------|---------|
| **Device Management** | `cpal`, `dashmap`, `tokio::sync` | Audio I/O, concurrent device tracking |
| **Audio Capture** | `crossbeam::channel`, `std::sync::atomic` | Lock-free communication, thread coordination |
| **VAD** | `vad-rs`, `ort`, `silero` | Voice activity detection engines |
| **STT** | `whisper-rs`, `deepgram`, `hf-hub` | Speech recognition engines |
| **Speaker ID** | `ort`, `ndarray`, `knf-rs` | ONNX inference, ML processing |
| **Database** | `sqlx`, `sqlite`, `fts5` | Type-safe storage, full-text search |
| **Intelligence** | `candle`, `tokenizers` | Local LLM processing, embeddings |

## Performance Metrics

- **Audio Latency**: ~2-5 seconds (segment duration + processing)
- **STT Accuracy**: 95%+ (Whisper Large), 98%+ (Deepgram)
- **Speaker Identification**: 90%+ accuracy with sufficient training data
- **Memory Usage**: ~200MB base + ~50MB per active device
- **Throughput**: Real-time processing up to 8 simultaneous devices
- **VAD Efficiency**: 70%+ silence filtering in typical environments

This pipeline processes continuous audio streams from multiple devices, extracting meaningful intelligence through advanced speech recognition, speaker identification, and semantic analysis while maintaining privacy through local processing capabilities.


# Screenpipe Multimodal Pipelines — Architecture, Audio Deep Dive & Library Map

*Updated: 2025‑08‑19*

## TL;DR
- **Goal:** Turn continuous **screen + audio** into a time‑aligned "scene graph" for search, agents, and recall.
- **Hot path:** Rust (deterministic latency, safe concurrency). **Platform shims:** Swift/Obj‑C (macOS), WinRT (Windows). **R&D/offline:** Python.
- **Key stages:** Capture → Filter (VAD/change) → Understand (OCR/STT/diarization) → Persist & Index (SQLite+FTS5+vectors) → Events → LLM/Agents.
- **This doc includes:** working Mermaid diagrams, an end‑to‑end overview, the complete Audio Pipeline deep dive, and the Core Libraries Analysis.
